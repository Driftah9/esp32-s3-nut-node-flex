...
- ups_get_report.c: add rid=0x85 to Eaton bootstrap probe queue
- Flags-based OL demoted: flags=0x0000 observed in all submissions including
  mains-loss events - only non-zero flags trigger OB
- OL now sourced from standard field cache (ACPresent/Charging/Discharging on
  vendor page 0xFFFF rids) or conservative charge-data default in derive_status()
...
## Last Action
2026-04-06 - v0.27: Eaton OL/OB fix: vendor page 0xFFFF, pre-seed OL, OB-only flags.
...
Build: clean. Also fixed in this session: CLAUDE.md Linux path corrections.

Previous: 2026-04-06 - v0.26: Eaton OB diagnostic improvements.
Eaton 0x8x alarm rids: log cap raised from 8 to 16 bytes, elevated to WARN.
rid=0x85 speculative BatterySystem probe added to decode_eaton_feature().
flags=0x0000 confirmed in all submissions - not reliable for OL assertion.

Previous: 2026-04-06 - v0.25b: extern C guard on app_main, IDF target v5.5.4.
Fix: IDF v5.5.x links via g++ - app_main gets C++ name-mangled without guard.
freertos startup looks for unmangled app_main -> link failure.
Added #ifdef __cplusplus / extern "C" / #endif guard around app_main definition.
IDF target string updated to v5.5.4 in startup banner.

Previous: 2026-04-06 - v0.25a: Fix send_set_idle() forward declaration ordering.
send_set_idle() placed before s_ctrl_done/s_ctrl_status/s_ctrl_bytes statics and
ctrl_transfer_cb() in ups_usb_hid.c. Incremental builds passed (stale .obj) but
clean builds failed with undeclared identifier errors. Moved send_set_idle() after
ctrl_transfer_cb() so all symbols are declared before use. Also replaced arrow
chars in comments with -> to avoid encoding issues.
