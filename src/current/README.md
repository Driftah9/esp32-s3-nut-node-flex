...
**Status:** Experimental / Pre-release - All three modes confirmed working (v0.41)
...
**Requires ESP-IDF v5.4.1 exactly.** Using a different version (including newer releases such as v6.x) will cause build failures. The IDF installer lets you select the version during setup.

Source is in `src/current/`.

```bash
# From src/current/
idf.py set-target esp32s3
idf.py build
idf.py -p PORT flash monitor   # replace PORT with your port (e.g. COM3 on Windows, /dev/ttyUSB0 on Linux)
```

If you already have a different IDF version installed, install v5.4.1 alongside it and use the v5.4.1 activation shortcut or script. Also delete the `build/` directory before rebuilding if you switch versions.
