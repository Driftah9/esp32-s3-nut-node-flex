                                     ...
                                     ups_state_update_t *upd)
{
    ...
    case 0x35:
         ...
         *   On mains:   input voltage × 10  (e.g. 2629 = 262.9V AC)
         *   On battery: battery voltage × 10 (e.g. 120 = 12.0V DC)
         *
         * The R20 note "2630 = 43m50s runtime" was a coincidence — the value
         * 2630 numerically matches both 263.0V × 10 and 2630 seconds.
         ...
         *
         * On battery, battery voltage × 10 = 120 gets misinterpreted by the
...
static void derive_status(ups_state_update_t *upd)
{
    ...
    if (!utility_known) {
        if (upd->battery_charge_valid) {
             ...
             * EXCEPTION: DECODE_VOLTRONIC has rid=0x32 (interrupt-IN) and
             * the QS command as authoritative status sources. Charge-only
             * reports (rid=0x34) arriving between status reports must not
             * overwrite a correct "OB" with "OL". Leave empty — no opinion. */
            ups_decode_mode_t mode = s_device ? s_device->decode_mode : DECODE_STANDARD;
            if (mode != DECODE_APC_SMARTUPS && mode != DECODE_VOLTRONIC) {
                                   ...
                                   ups_state_update_t *upd)
{
    ...
    if (mode == DECODE_CYBERPOWER) {
        ...
        /* QS command (every 2s) and QI command (every 10s) are the sole
         * data sources for all measurements on this device.
         * The only thing we decode from interrupt-IN HID is rid=0x32
         * (fast OL/OB status — 512ms vs 2s QS poll interval).
         * All other rids and the entire standard descriptor path are
         * blocked — they write stale or wrong data that conflicts with QS/QI. */
