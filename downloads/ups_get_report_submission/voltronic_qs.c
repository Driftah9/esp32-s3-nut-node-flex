...
/* Command types */
typedef enum {
    QS_CMD_QUERY    = 0,   /* QS\r  - live voltage/load/status         */
    QS_CMD_RATINGS  = 1,   /* F\r   - nominal ratings (one-shot/retry) */
} qs_cmd_t;
 ...
 * response, etc.) s_nom_batt_volt stays 0 — blocking ALL runtime estimation.
...
static void parse_qs_response(const char *resp)
{
    ...
    int nfields = 0;
    char *saveptr = NULL;
    for (char *tok = strtok_r(buf, " ", &saveptr); tok && nfields < 10;
         tok = strtok_r(NULL, " ", &saveptr)) {
        fields[nfields++] = tok;
    }

    /* Need at least 8 fields: inV inVfault outV load Hz battV temp flags.
     * If fewer arrive the USB chunk boundary ate part of the response.
     * Safe action: discard and wait for the next poll (2s later).
     * Do NOT attempt field-shift recovery — when on battery inV=000.0
     * and outV=234.7, a shifted parse misidentifies outV as inV and
     * maps NULL to flags, defaulting to util_fail=false → false OL. */
    if (nfields < 8) {
        ESP_LOGW(TAG, "QS parse: only %d fields (need 8) - discarding truncated response",
                 nfields);
        return;
    }
    ...
    /* Decode utility-fail FIRST — needed to make the inV sanity check safe.
     * When util_fail=true (on battery) inV=0.0V is correct and expected:
     * the utility is absent. Only skip on low inV when we believe we are
     * still on mains — that situation indicates a truncated/shifted response. */
    bool util_fail   = (strlen(flags) >= 1 && flags[0] == '1');

    /* Sanity: if on mains but inV < 50V, response was likely truncated or
     * the fields are shifted (battery voltage landed in field[0]).
     * Do NOT apply this guard when on battery — inV=0 is valid there. */
    if (!util_fail && inV < 50.0f) {
        ESP_LOGW(TAG, "QS parse: inV=%.1f too low while on mains (truncated response?) - skipping", inV);
        return;
    }
     ...
     * Confirmed observations on this 24V UPS:
     *   On mains (float):  battV ≈ 27.2V  → 100%
     *   Fresh off mains:   battV ≈ 26.5V  → ~90%
     *   Mid discharge:     battV ≈ 26.0V  → ~60%
     *   Low battery:       battV ≈ 25.0V  → ~20%
     *   Cutoff:            battV ≈ 21.0V  → 0%
     *
     * Voltage range per cell for sealed lead-acid:
     *   Float charge:  ~2.27V/cell  → ×12 cells = 27.2V for 24V pack
...
/* ---- Parse QI (internals) response ---- */
static void parse_qi_response(const char *resp)
{
    /* Format confirmed on VID=0665 PID=5161 (saper-2/py-upshid-voltqs):
     *   (batt_pct runtime_s freq ??? ??? ??? ??? flags\r
     *
     *   field[0] = battery charge %  (3 digits, e.g. "099" = 99%)
     *   field[1] = runtime remaining in seconds (5 digits, e.g. "02920" = 2920s)
     *   field[2] = output frequency Hz
     *   field[3..6] = unknown (thresholds / config values)
     *   field[7] = 16-char status flags
     *
     * This gives EXACT firmware values — no estimation needed. */
    const char *s = resp;
    while (*s == '(' || *s == ' ') s++;
    ...
    if (nfields < 2) {
        ESP_LOGW(TAG, "QI parse: only %d fields (need 2): %s", nfields, resp);
        return;
    }

    int      charge     = atoi(fields[0]);
    uint32_t runtime_s  = (uint32_t)atol(fields[1]);

    if (charge < 0 || charge > 100) {
        ESP_LOGW(TAG, "QI parse: charge=%d out of range", charge);
        return;
    }

    /* Sanity check: runtime=0 means UPS hasn't calculated it yet (normal
     * immediately after switching to battery). Accept it — charge% is still valid.
     * Reject only if runtime is suspiciously small but non-zero: under 120s
     * (2 minutes) is physically impossible for a UPS with any meaningful charge.
     * The "50s" artifact seen in logs is a truncated/partial USB response. */
    if (runtime_s > 0u && runtime_s < 120u) {
        ESP_LOGW(TAG, "QI parse: charge=%d%% runtime=%"PRIu32"s too short "
                 "(< 120s minimum) - discarding truncated response",
                 charge, runtime_s);
        return;
    }
    ...
    upd.battery_charge_valid = true;
    upd.battery_charge       = (uint8_t)charge;

    /* runtime_s=0 means UPS hasn't calculated it yet (e.g. just connected).
     * Only apply when > 0 and within a sane range. */
    if (runtime_s > 0u && runtime_s < 72000u) {  /* cap at 20h */
        upd.battery_runtime_valid = true;
    ...
    ESP_LOGI(TAG, "[QI] battery.charge=%d%% runtime=%"PRIu32"s (~%"PRIu32"m%02"PRIu32"s)",
             charge, runtime_s, runtime_s / 60u, runtime_s % 60u);
}
