...
/* Voltronic/PowerWalker: no Feature reports polled.
 * QS command (every 2s) provides: inV, outV, load%, battV, freq, temp, OL/OB.
 * QI command (every 10s) provides: battery.charge%, runtime seconds.
 * Feature reports 0x21/0x34/0x36 are static registers that do not update
 * in real time and conflict with the live QI/QS values. Removed entirely. */
static const uint8_t s_voltronic_rids[]    = { };
static const size_t  s_voltronic_rids_n    = 0;
...
static void decode_voltronic_feature(uint8_t rid, const uint8_t *buf, size_t len)
{
    ...
    case 0x21:
         ...
         * It is kept here only as a last-resort fallback if the QI command
         * is not supported. QI (polled every 10s) is the authoritative source
         * for battery.charge and battery.runtime on this device.
         *
         * No runtime estimate here — QI provides the real firmware value. */
